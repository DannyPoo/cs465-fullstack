...
export class AuthenticationService {
  ...
  // Note: for this application we have decided we will name
  ...
  // the eky for our token 'travlr-token'
  ...
  //Boolean to determine if we are logged ina ndt he token is
  // still valid. Even if we have a token we will still have to
  // reauthenticate if the token has expired
  public isLoggedIn(): boolean {
    const token: string = this.getToken();
    if (token){
      const paylord = JSON.parse(atob(token.split('.')[1]));
    return paylord.exp > (Date.now() / 1000);
    } else {
      return false;
    }
  }

  // Retrieve the current user. This funciton should only be caleld
  ...
  // result and only process when the Obseravable condition is satisfied
  // Uncomment the two console.log messages for addtion debugging
  // information.
